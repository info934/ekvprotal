...
import { AlertTriangle, CalendarClock, Camera, CheckCircle2, ChevronRight, Inbox, List, Mail, Paperclip, Plus, Search, ShieldCheck, Wrench } from 'lucide-react';
...
const Service = () => {
  ...
  useEffect(() => {
    if (searchParams.get('new') === 'case' && canEdit) {
      setDraft(blankCase); setDialogOpen(true); setSearchParams({}, { replace: true });
    }
  }, [canEdit, searchParams, setSearchParams]);

  const load = useCallback(async () => {
    setLoading(true);
    supabase.rpc('refresh_service_sla_alerts').then(() => {}).catch(() => {});
  ...
  return <div className="app-page space-y-5">
    ...
    <section className="app-surface overflow-hidden">
      <div className="flex flex-wrap items-center gap-2 border-b p-4"><Button type="button" size="sm" variant={display === 'list' ? 'default' : 'outline'} onClick={() => setDisplay('list')}><List className="mr-2 h-4 w-4" />Seznam</Button><Button type="button" size="sm" variant={display === 'calendar' ? 'default' : 'outline'} onClick={() => setDisplay('calendar')}><CalendarClock className="mr-2 h-4 w-4" />Plán techniků</Button></div>
      ...
      {display === 'calendar' ? <div className="grid gap-3 p-4 md:grid-cols-2 xl:grid-cols-3">{filtered.filter((item) => item.scheduled_start).sort((a,b) => new Date(a.scheduled_start) - new Date(b.scheduled_start)).map((item) => <Link key={item.id} to={`/service/${item.id}`} className="rounded-xl border bg-white p-4 transition hover:border-blue-300 hover:shadow-sm"><div className="flex items-start justify-between gap-3"><div><p className="text-xs font-semibold uppercase tracking-wide text-blue-700">{formatServiceDate(item.scheduled_start)}</p><h3 className="mt-1 font-semibold">{item.number} · {item.client_name}</h3></div><span className={`rounded-full px-2 py-1 text-xs font-semibold ${priorityTone(item.priority)}`}>{servicePriorityLabels[item.priority]}</span></div><p className="mt-2 text-sm text-slate-600">{item.title}</p><p className="mt-3 text-xs text-slate-500">Technik: {item.assigned?.name || 'Nepřiřazeno'}</p></Link>)}{!filtered.some((item) => item.scheduled_start) && <div className="col-span-full p-8 text-center text-sm text-slate-500">Ve zvoleném filtru nejsou naplánované žádné servisní výjezdy.</div>}</div> : <><div className="hidden overflow-x-auto lg:block"><table className="w-full text-sm"><thead><tr className="bg-slate-50 text-left text-xs uppercase tracking-wide text-slate-500"><th className="px-4 py-3">Případ / klient</th><th className="px-4 py-3">Systém a vazba</th><th className="px-4 py-3">Stav</th><th className="px-4 py-3">Priorita</th><th className="px-4 py-3">Technik</th><th className="px-4 py-3">Naplánováno</th><th className="px-4 py-3">Dokumentace</th><th /></tr></thead><tbody>
