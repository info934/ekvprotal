...
import { AlertTriangle, CalendarClock, Camera, CheckCircle2, ChevronRight, Plus, Search, ShieldCheck, Wrench } from 'lucide-react';
...
const Service = () => {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    const [caseRes, memberRes, subjectRes, opportunityRes, realizationRes, projectRes] = await Promise.all([
    ...
    const error = caseRes.error || memberRes.error || subjectRes.error || opportunityRes.error || realizationRes.error || projectRes.error;
    if (error) toast({ title: 'Servis se nepodařilo načíst', description: error.message, variant: 'destructive' });
    setCases(caseRes.data || []); setMembers(memberRes.data || []); setSubjects(subjectRes.data || []);
  ...
  const createCase = async (event) => {
    ...
    const { data, error } = await supabase.rpc('create_service_case', { p_payload: payload });
    setSaving(false);
    if (error) return toast({ title: 'Případ se nepodařilo vytvořit', description: error.message, variant: 'destructive' });
    toast({ title: `${data.number} byl vytvořen` }); setDialogOpen(false); setDraft(blankCase); navigate(`/service/${data.id}`);
  };
  ...
  return <div className="app-page space-y-5">
    <PageHeader icon={Wrench} title="Servis a reklamace" description="Řízení reklamací, servisních výjezdů, fotodokumentace a podepsaných protokolů."
      actions={canEdit && <Button onClick={() => setDialogOpen(true)}><Plus className="mr-2 h-4 w-4" />Nový servisní případ</Button>} />
    ...
    <Dialog open={dialogOpen} onOpenChange={setDialogOpen}><DialogContent className="max-h-[92vh] max-w-3xl overflow-y-auto"><DialogHeader><DialogTitle>Nový servisní případ</DialogTitle><DialogDescription>Založte reklamaci nebo servis z existující zakázky, případně jako samostatný případ.</DialogDescription></DialogHeader><form onSubmit={createCase} className="space-y-5">
      ...
      <DialogFooter><Button type="button" variant="outline" onClick={() => setDialogOpen(false)}>Zrušit</Button><Button disabled={saving}>{saving ? 'Zakládám…' : 'Založit případ'}</Button></DialogFooter>
