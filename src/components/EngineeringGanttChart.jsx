...
const EngineeringGanttChart = ({ activities }) => {
  ...
  if (!validActivities || validActivities.length === 0) {
    return (
      <div className="text-center py-8 text-muted-foreground bg-slate-50 rounded-lg">
        <p>Žádné činnosti s termínem k zobrazení.</p>
        <p className="text-sm">Nastavte činnostem datum zahájení a dokončení pro zobrazení v časové ose.</p>
      </div>
    );
  }
  ...
  const overallStartDate = startOfDay(new Date(Math.min(...validActivities.map(a => parseISO(a.start_date).getTime()))));
  const overallEndDate = startOfDay(new Date(Math.max(...validActivities.map(a => parseISO(a.end_date).getTime()))));
    ...
    <div className="space-y-3 relative p-4 bg-slate-50 rounded-lg overflow-x-auto">
      <div className="w-full h-full relative" style={{ minWidth: '800px' }}>
          ...
          <div className="flex items-center sticky top-0 bg-slate-50 z-20 pb-2">
              <div className="w-[250px] pr-4 font-semibold text-muted-foreground">Činnost</div>
          ...
          <div className="space-y-2">
              {sortedActivities.map((activity, index) => {
                  const startDate = startOfDay(parseISO(activity.start_date));
                  const endDate = startOfDay(parseISO(activity.end_date));
                  ...
                  const barColor = getBarColor(activity);
                  ...
                  return (
                    ...
                    >
                      <div className="w-[250px] pr-4 truncate">
                          <p className="font-medium truncate text-slate-800 text-sm">{activity.subject}</p>
                      ...
                      <div className="flex-1 relative h-full">
                          <div className="relative h-full w-full bg-slate-200 rounded-sm">
                            <motion.div
                              ...
                              title={`${activity.subject}: ${format(startDate, 'd.M.yy')} - ${format(endDate, 'd.M.yy')}`}
                            >
...
export default EngineeringGanttChart;