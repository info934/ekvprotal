...
export default function AttendancePlanning({ memberId }) {
  ...
  return <div className="space-y-5">
    ...
    <div className="flex flex-wrap items-center justify-between gap-3"><AttendanceMonthControl value={month} disabled={pending} onChange={value => setParams(current => { const next = new URLSearchParams(current); next.set('month', format(value, 'yyyy-MM')); return next; })} />
      {manage && <label className="flex w-full flex-col gap-1 text-sm sm:w-auto sm:flex-row sm:items-center">Pracovník<select aria-label="Pracovník pro plán docházky" disabled={pending || members.loading} className="min-h-11 w-full rounded-md border bg-white px-3 py-2 sm:ml-2 sm:w-[240px]" value={target || ''} onChange={event => { setSelectedMember(event.target.value); setSelectedDay(null); setError(''); }}><option value={memberId || ''}>Moje plánování</option>{(members.data || []).filter(row => row.id !== memberId).map(row => <option key={row.id} value={row.id}>{row.name}</option>)}</select></label>}
    </div>
    ...
    <Dialog open={Boolean(draft)} onOpenChange={open => { if (!open && !pending) setDraft(null); }}><DialogContent><DialogHeader><DialogTitle>{draft?.version ? 'Upravit plán' : 'Naplánovat docházku'}</DialogTitle><DialogDescription>Plán se nezapočítává do odpracovaných hodin. Směnu přes půlnoc rozdělte do dvou dnů.</DialogDescription></DialogHeader>{draft && <form onSubmit={save} className="grid grid-cols-1 gap-4 sm:grid-cols-2"><label className="text-sm sm:col-span-2">Datum<Input required type="date" disabled={pending} value={draft.date} onChange={event=>setDraft({...draft,date:event.target.value})}/></label><label className="text-sm">Od<Input required type="time" disabled={pending} value={draft.start} onChange={event=>setDraft({...draft,start:event.target.value})}/></label><label className="text-sm">Do<Input required type="text" placeholder="16:30 nebo 24:00" pattern="([01][0-9]|2[0-3]):[0-5][0-9]|24:00" disabled={pending} value={draft.end} onChange={event=>setDraft({...draft,end:event.target.value})}/></label><label className="text-sm">Přestávka (min)<Input required type="number" min="0" max="1439" step="1" disabled={pending} value={draft.break_minutes} onChange={event=>setDraft({...draft,break_minutes:event.target.value})}/></label><label className="text-sm">Typ<select className="mt-1 min-h-11 w-full rounded-md border p-2" disabled={pending} value={draft.kind} onChange={event=>setDraft({...draft,kind:event.target.value})}>{Object.entries(PLAN_LABELS).map(([value,label])=><option key={value} value={value}>{label}</option>)}</select></label><label className="text-sm sm:col-span-2">Místo / zakázka / poznámka<Textarea maxLength={1000} disabled={pending} value={draft.note} onChange={event=>setDraft({...draft,note:event.target.value})}/></label>{error && <p role="alert" className="text-sm text-red-700 sm:col-span-2">{error}</p>}<div className="flex flex-col gap-2 sm:col-span-2 sm:flex-row"><Button type="submit" disabled={pending}>{pending?'Ukládám…':'Uložit plán'}</Button><Button type="button" variant="outline" disabled={pending} onClick={()=>setDraft(null)}>Zavřít</Button></div></form>}</DialogContent></Dialog>
