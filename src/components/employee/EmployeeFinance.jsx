...
export default function EmployeeFinance({ memberId: targetMemberId }) {
  ...
  return <div className="space-y-5">
    <div className="flex flex-wrap items-start justify-between gap-3"><div><h2 className="text-lg font-semibold text-slate-900">{isOwn ? 'Moje finance' : 'Finance zaměstnance'}</h2><p className="mt-1 text-sm text-slate-500">Osobní sazba, nároky ze zakázek a stav výplat. Částky respektují soukromý režim.</p></div><div className="flex flex-wrap gap-2"><Button asChild variant="outline"><Link to="/payouts">Přejít do výplat<ArrowUpRight className="ml-2 h-4 w-4" /></Link></Button>{hasPermission('members', 'can_read') && <Button asChild variant="ghost"><Link to={`/members/${targetMemberId}`}>Původní detail člena</Link></Button>}<Button variant="ghost" size="icon" aria-label="Obnovit finance" onClick={() => setReload(current => current + 1)}><RefreshCw className="h-4 w-4" /></Button></div></div>
