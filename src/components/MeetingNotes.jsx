...
export default function MeetingNotes({ entityType, entityId, entityTitle, entityCode, canEdit, onOpenPlan }) {
  const [state,setState]=useState({ loading:true,rows:[],items:[],documents:{},planId:null,error:'' });
  ...
  const load=useCallback(async()=>{
    ...
    try {
      ...
      let documents = [];
      if (rows.length) {
        const result = await supabase.from('meeting_note_documents').select('*').in('note_id', rows.map(row=>row.id));
        if (result.error && result.error.code !== '42P01') throw result.error;
        documents = result.data || [];
      }
      const documentMap = Object.fromEntries(documents.map(document=>[document.note_id,document]));
      if(id===request.current) setState({loading:false,rows,items,documents:documentMap,planId:plan?.plan_id||null,error:''});
  ...
  return <section className="space-y-4">
    ...
    <p className="rounded-lg border bg-slate-50 p-3 text-sm text-slate-600">Zápis zůstává interní, dokud jej neodešlete. Při exportu se firemní PDF automaticky uloží do složky zápisů s datem a verzí v názvu.</p>
    ...
    {printNote&&<MeetingPrintDialog note={printNote} entityTitle={entityTitle} entityType={entityType} entityId={entityId} entityCode={entityCode} canEdit={canEdit} onChanged={load} onClose={()=>setPrintNote(null)}/>}
  </section>;
}
