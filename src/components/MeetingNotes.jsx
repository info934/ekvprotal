...
export default function MeetingNotes({ entityType, entityId, canEdit, onOpenPlan }) {
  ...
  const prepareTask=async(row,index)=>{setActionError('');setMembers([]);setTaskDraft({row,index,name:row.points[index].text.slice(0,200),due:new Date().toLocaleDateString('sv-SE'),memberId:''}); const {data,error:failure}=await supabase.rpc('list_planning_members_safe',{p_plan_id:row.plan_id});if(failure)setActionError(failure.message);else setMembers(data||[]);};
  ...
  const followUp=row=>{open();setDraft({id:crypto.randomUUID(),version:0,title:row.title,meeting_date:new Date().toLocaleDateString('sv-SE'),participants:row.participants,points:row.points.filter(p=>p.planning_item_id&&state.items.some(i=>i.id===p.planning_item_id&&!['done','cancelled'].includes(i.status))).map(p=>({...p}))});};
