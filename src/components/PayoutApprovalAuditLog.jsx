...
import { getPayoutApprovalHistory } from '@/lib/PayoutApprovalService';
import { Loader2, History, User, CheckCircle2, FileWarning } from 'lucide-react';
import { format } from 'date-fns';
import { cs } from 'date-fns/locale';

const PayoutApprovalAuditLog = ({ payoutId }) => {
  ...
  useEffect(() => {
    const fetchLogs = async () => {
      if (!payoutId) return;
      setLoading(true);
      const result = await getPayoutApprovalHistory(payoutId);
      if (result.success) {
        setLogs(result.data.filter(log => log.action.startsWith('payout_approved')));
      }
      setLoading(false);
    };
...
export default PayoutApprovalAuditLog;