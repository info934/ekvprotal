...
import { Plus, Trash2, CalendarPlus as CalendarIcon, Send, Mail, ChevronLeft, ShoppingCart } from 'lucide-react';
...
const RealizaceOrderForm = () => {
        ...
        if (isEditing) {
            ...
            if (orderData) {
                setFormData({
                    ...
                    items: orderData.items || [{ description: '', quantity: 1, unit: 'ks', unit_price: 0, total_price: 0 }],
    ...
    const addItem = () => {
        setFormData(prev => ({
            ...prev,
            items: [...prev.items, { description: '', quantity: 1, unit: 'ks', unit_price: 0, total_price: 0 }],
        }));
    };
    ...
    const handleSendEmail = async () => {
        ...
        try {
            await sendEmail({
                to: validEmails.join(','),
                subject: `Objednávka č. ${formData.order_number}`,
                greeting: `Dobrý den,`,
        ...
        <div className="p-2 sm:p-4 md:p-6 space-y-6">
            ...
            <Card>
                ...
                <CardContent className="space-y-6 pt-6">
                    ...
                    <div className="space-y-2">
                        <Label className="text-base">Položky</Label>
                            ...
                            <Table>
                                <TableHeader>
                                    <TableRow>
                                        ...
                                        <TableHead className="w-28">Jednotka</TableHead>
                                        <TableHead className="w-36">Cena/jedn.</TableHead>
                                ...
                                <TableBody>
                                    {formData.items.map((item, index) => (
                                        <TableRow key={index}>
                                            <TableCell><Input placeholder="Např. Cihly Heluz" value={item.description} onChange={(e) => handleItemChange(index, 'description', e.target.value)} className="h-11" /></TableCell>
                                            <TableCell><Input type="number" value={item.quantity} onChange={(e) => handleItemChange(index, 'quantity', e.target.value)} className="h-11" /></TableCell>
                                            <TableCell><Input value={item.unit} onChange={(e) => handleItemChange(index, 'unit', e.target.value)} className="h-11" /></TableCell>
                                            <TableCell><Input type="number" placeholder="0" value={item.unit_price} onChange={(e) => handleItemChange(index, 'unit_price', e.target.value)} className="h-11" /></TableCell>
                                            <TableCell className="text-right font-medium text-base">{(item.total_price || 0).toLocaleString('cs-CZ')} Kč</TableCell>
                                            <TableCell><Button variant="ghost" size="icon" onClick={() => removeItem(index)}><Trash2 className="w-5 h-5 text-red-500" /></Button></TableCell>
                                        </TableRow>
                                    ))}
                                </TableBody>
                            </Table>
                        </div>
                        <Button variant="outline" size="sm" onClick={addItem} className="mt-2 text-base py-2 px-4 h-auto"><Plus className="w-4 h-4 mr-2" />Přidat položku</Button>
                    </div>

                    <div className="text-right font-bold text-2xl">Celkem: {formData.total_amount.toLocaleString('cs-CZ')} Kč</div>
