...
export default function UnifiedTaskQueue(){
 ...
 const search=(params.get('q')||'').slice(0,250);
 ...
 <div className="flex flex-wrap items-center justify-between gap-2 text-sm text-slate-500"><span role="status">{state.loading?'Načítám úkoly…':state.error?'Počet úkolů není dostupný':`Zobrazeno úkolů: ${rows.length}`}</span>{(scope!=='open'||kind!=='all'||search)&&<Button variant="ghost" size="sm" onClick={reset}>Zrušit filtry</Button>}</div>
