...
const formatCurrency = (value) => new Intl.NumberFormat('cs-CZ', { style: 'currency', currency: 'CZK', maximumFractionDigits: 0 }).format(Number(value || 0));
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-5xl">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2"><Calculator className="h-5 w-5 text-primary" /> Jednoduchá FVE nabídka</DialogTitle>
        </DialogHeader>
        <div className="grid gap-5 lg:grid-cols-[320px_minmax(0,1fr)]">
          <div className="space-y-4 rounded-xl border bg-slate-50/70 p-4">
            <div className="space-y-1.5">
              <Label>Výkon FVE (kWp)</Label>
              <Input type="number" min="0" step="0.1" value={inputs.power_kwp} onChange={(event) => update('power_kwp', Number(event.target.value || 0))} />
            </div>
            <div className="space-y-1.5">
              <Label>Kapacita baterie (kWh)</Label>
              <Input type="number" min="0" step="0.1" value={inputs.battery_kwh} onChange={(event) => update('battery_kwh', Number(event.target.value || 0))} />
            </div>
            <div className="space-y-1.5">
            ...
            <div className="flex items-center justify-between rounded-lg border bg-white px-3 py-2">
              ...
              <Switch id="fve-wallbox" checked={inputs.include_wallbox} onCheckedChange={(value) => update('include_wallbox', value)} />
            </div>
            <div className="rounded-lg border bg-white p-3 text-sm">
              ...
              <p className="mt-1 text-xs text-slate-500">Položky lze po vložení normálně ručně upravit.</p>
            </div>
          </div>
          ...
          <div className="space-y-3">
            ...
            <div className="max-h-[430px] overflow-auto rounded-lg border">
              <Table>
                <TableHeader><TableRow><TableHead>Kód</TableHead><TableHead>Název</TableHead><TableHead className="text-right">Množství</TableHead><TableHead className="text-right">Cena</TableHead><TableHead className="text-right">Celkem</TableHead></TableRow></TableHeader>
                <TableBody>
                  {loading ? <TableRow><TableCell colSpan={5} className="py-8 text-center"><Loader2 className="mx-auto h-5 w-5 animate-spin" /></TableCell></TableRow> : items.map((item) => (
                    <TableRow key={item.id || item.code}>
                      <TableCell className="font-mono text-xs">{item.code || '-'}</TableCell>
                      <TableCell><div className="font-medium">{item.name}</div>{item.description && <div className="text-xs text-slate-500">{item.description}</div>}</TableCell>
                      <TableCell className="text-right tabular-nums">{Number(item.quantity || 0).toLocaleString('cs-CZ')} {item.unit}</TableCell>
            ...
            <div className="grid gap-2 text-sm sm:grid-cols-3">
              <div className="rounded-lg border bg-white p-3"><span className="text-slate-500">Bez DPH</span><div className="font-semibold">{formatCurrency(totals.total)}</div></div>
              <div className="rounded-lg border bg-white p-3"><span className="text-slate-500">DPH</span><div className="font-semibold">{formatCurrency(totals.tax_total)}</div></div>
              <div className="rounded-lg border bg-primary p-3 text-primary-foreground"><span>Celkem</span><div className="font-semibold">{formatCurrency(totals.total + totals.tax_total)}</div></div>
            </div>
          </div>
        </div>
        <DialogFooter>
          <Button variant="outline" onClick={() => onOpenChange(false)}>Zrušit</Button>
          <Button onClick={() => onApply(items)} disabled={loading || items.length === 0}><Check className="mr-2 h-4 w-4" />Vložit položky</Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
  );
