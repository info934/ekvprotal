...
const ServiceDetail = () => {
  ...
  const saveVisit = async (event) => {
    ...
    const request = visitDraft.id ? supabase.from('service_visits').update(payload).eq('id', visitDraft.id) : supabase.from('service_visits').insert(payload);
    const { error } = await request; setSavingVisit(false);
    if (error) return toast({ title: 'Výjezd se nepodařilo uložit', description: error.message, variant: 'destructive' });
  ...
  const uploadPhotos = async (event) => {
    ...
    for (const file of files) {
      if (!file.type.startsWith('image/') || file.size > 15 * 1024 * 1024) { toast({ title: `${file.name} nebyl nahrán`, description: 'Povoleny jsou obrázky do 15 MB.', variant: 'destructive' }); continue; }
      ...
      if (uploadError) { toast({ title: `${file.name} se nepodařilo nahrát`, description: uploadError.message, variant: 'destructive' }); continue; }
  ...
  return <div className="app-page space-y-5 pb-24 md:pb-8">
    ...
    <Dialog open={visitOpen} onOpenChange={setVisitOpen}><DialogContent className="max-h-[94vh] max-w-3xl overflow-y-auto"><DialogHeader><DialogTitle>{visitDraft?.id ? `Výjezd #${visitDraft.visit_number}` : 'Nový servisní výjezd'}</DialogTitle><DialogDescription>Formulář je připravený pro telefon i tablet. Měření a materiál zapisujte po jednom řádku.</DialogDescription></DialogHeader>{visitDraft && <form onSubmit={saveVisit} className="space-y-5"><div className="grid gap-4 sm:grid-cols-3"><div className="space-y-2"><Label>Vedoucí technik</Label><select className={selectClass} value={visitDraft.lead_technician_id} onChange={(e) => setVisit('lead_technician_id', e.target.value)}><option value="">Nepřiřazeno</option>{members.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></div><div className="space-y-2"><Label>Začátek</Label><Input type="datetime-local" value={visitDraft.scheduled_start} onChange={(e) => setVisit('scheduled_start', e.target.value)} /></div><div className="space-y-2"><Label>Konec</Label><Input type="datetime-local" value={visitDraft.scheduled_end} onChange={(e) => setVisit('scheduled_end', e.target.value)} /></div></div><div className="grid gap-4 sm:grid-cols-2"><div className="space-y-2"><Label>Diagnostika</Label><Textarea className="min-h-[100px]" value={visitDraft.diagnostics} onChange={(e) => setVisit('diagnostics', e.target.value)} placeholder="Alarmy, stav zařízení, vzdálená diagnostika…" /></div><div className="space-y-2"><Label>Zjištěná příčina</Label><Textarea className="min-h-[100px]" value={visitDraft.root_cause} onChange={(e) => setVisit('root_cause', e.target.value)} /></div></div><div className="space-y-2"><Label>Provedené práce</Label><Textarea className="min-h-[120px]" value={visitDraft.work_performed} onChange={(e) => setVisit('work_performed', e.target.value)} placeholder="Popište zásah v pořadí, v jakém byl proveden." /></div><div className="grid gap-4 sm:grid-cols-2"><div className="space-y-2"><Label>Použitý materiál</Label><Textarea value={visitDraft.materials_text} onChange={(e) => setVisit('materials_text', e.target.value)} placeholder={'Pojistka DC | 2 | ks\nKonektor MC4 | 4 | ks'} /><p className="text-xs text-slate-500">Název | množství | jednotka</p></div><div className="space-y-2"><Label>Měření</Label><Textarea value={visitDraft.measurements_text} onChange={(e) => setVisit('measurements_text', e.target.value)} placeholder={'Napětí stringu 1 | 612 | V\nIzolační odpor | 128 | MΩ'} /><p className="text-xs text-slate-500">Měření | hodnota | jednotka</p></div></div><fieldset className="rounded-xl border p-4"><legend className="px-2 text-sm font-semibold">Bezpečnostní a funkční kontrola</legend><div className="grid gap-3 sm:grid-cols-2">{visitDraft.safety_checks.map((item,index) => <label key={`${item.label}-${index}`} className="flex min-h-11 items-center gap-3 rounded-lg bg-slate-50 px-3 text-sm"><input type="checkbox" className="h-5 w-5 rounded" checked={Boolean(item.checked)} onChange={(e) => setVisit('safety_checks', visitDraft.safety_checks.map((check,i) => i === index ? { ...check, checked:e.target.checked } : check))} />{item.label}</label>)}</div></fieldset><div className="grid gap-4 sm:grid-cols-2"><div className="space-y-2"><Label>Doporučení klientovi</Label><Textarea value={visitDraft.recommendations} onChange={(e) => setVisit('recommendations', e.target.value)} /></div><div className="space-y-2"><Label>Další krok</Label><Textarea value={visitDraft.next_action} onChange={(e) => setVisit('next_action', e.target.value)} placeholder="Prázdné znamená, že lze případ vyřešit." /></div></div><div className="space-y-2"><Label>Vyjádření klienta</Label><Textarea value={visitDraft.client_statement} onChange={(e) => setVisit('client_statement', e.target.value)} /></div><label className="flex min-h-12 items-center gap-3 rounded-lg border p-3 text-sm font-medium"><input type="checkbox" className="h-5 w-5" checked={visitDraft.client_present} onChange={(e) => setVisit('client_present', e.target.checked)} />Klient byl zásahu přítomen</label><DialogFooter><Button type="button" variant="outline" onClick={() => setVisitOpen(false)}>Zrušit</Button><Button disabled={savingVisit}>{savingVisit ? 'Ukládám…' : 'Uložit výjezd'}</Button></DialogFooter></form>}</DialogContent></Dialog>
