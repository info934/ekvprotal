...
const AdminPayoutApprovalDialog = ({ isOpen, onClose, payout, onConfirm }) => {
    ...
    try {
      ...
      if (notifications.some(result => result.status === 'rejected' || !result.value?.success)) {
        toast({
          ...
          description: 'Stav je uložený. Některý e-mail se nepodařilo potvrdit; schválení neopakujte.',
          variant: 'warning'
        });
      } else {
        toast({
  ...
  return (
    <Dialog open={isOpen} onOpenChange={(open) => !open && !pending.current && onClose()}>
      <FormDialogContent size="md">
        ...
        <FormDialogBody className="space-y-6">
          <div className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
            <div className="grid gap-3 sm:grid-cols-3">
              ...
              <div className="rounded-lg bg-emerald-50 p-3">
                ...
                <div className="mt-1 font-bold text-emerald-950"><FinanceAmount value={payout.amount} exact /></div>
              </div>
            </div>
          </div>
          ...
          <Alert className="border-blue-200 bg-blue-50 text-blue-900">
            ...
            <AlertDescription>
              Po schválení bude žádost čekat na fakturu od zaměstnance. Po jejím nahrání a skutečné úhradě administrátor zaznamená vyplacení. Schválení samo peníze neodesílá.
            </AlertDescription>
          </Alert>
