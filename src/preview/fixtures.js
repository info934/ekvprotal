...
export function createFixtures() {
  ...
  const service_cases = [
    { id: uuid(92001), number: 'SRV-2026-0001', title: 'BESS hlásí chybu komunikace', case_kind: 'complaint', system_type: 'fve_bess', status: 'in_progress', priority: 'critical', warranty_status: 'in_warranty', source: 'client', realizace_id: realizations[0].id, project_id: projects[1].id, subject_id: subjects[1].id, assigned_member_id: ADMIN_ID, client_name: subjects[1].name, client_contact_name: 'Pavel Novotný', client_email: subjects[1].email, client_phone: '+420 000 000 000', installation_address: realizations[0].location_address, description: 'Bateriové úložiště se odpojuje od střídače a v monitoringu se opakuje alarm komunikace BMS.', equipment_summary: 'FVE 49,5 kWp · BESS 60 kWh · hybridní střídač', error_code: 'BMS-COM-031', scheduled_start: timestamp(1), scheduled_end: `${previewDate(1)}T11:30:00Z`, reported_at: timestamp(-1), created_at: timestamp(-1), updated_at: timestamp(0) },
    { id: uuid(92002), number: 'SRV-2026-0002', title: 'Pokles výroby stringu č. 3', case_kind: 'service', system_type: 'fve', status: 'scheduled', priority: 'high', warranty_status: 'unknown', source: 'monitoring', opportunity_id: crm_opportunities[1].id, subject_id: subjects[0].id, assigned_member_id: MEMBER_ID, client_name: subjects[0].name, client_contact_name: 'Martin Novotný', client_email: subjects[0].email, client_phone: '+420 000 000 000', installation_address: subjects[0].address, description: 'Monitoring ukazuje přibližně o 35 % nižší výkon stringu č. 3 proti ostatním stringům.', equipment_summary: 'FVE 12 kWp · střídač 10 kW', scheduled_start: timestamp(2), scheduled_end: `${previewDate(2)}T13:00:00Z`, reported_at: timestamp(-2), created_at: timestamp(-2), updated_at: timestamp(-1) },
    { id: uuid(92003), number: 'SRV-2026-0003', title: 'Preventivní kontrola bateriového úložiště', case_kind: 'maintenance', system_type: 'bess', status: 'resolved', priority: 'normal', warranty_status: 'out_of_warranty', source: 'internal', subject_id: subjects[2].id, assigned_member_id: uuid(3), client_name: subjects[2].name, client_contact_name: 'Správa budovy', client_email: subjects[2].email, installation_address: subjects[2].address, description: 'Roční kontrola stavu bateriových modulů, BMS a chlazení.', equipment_summary: 'BESS 120 kWh', resolution_summary: 'Kontrola dokončena bez závad.', scheduled_start: timestamp(-5), scheduled_end: `${previewDate(-5)}T12:00:00Z`, reported_at: timestamp(-10), resolved_at: timestamp(-5), created_at: timestamp(-10), updated_at: timestamp(-5) },
  ];
  const service_visits = [
    { id: uuid(92101), service_case_id: service_cases[0].id, visit_number: 1, status: 'in_progress', scheduled_start: service_cases[0].scheduled_start, scheduled_end: service_cases[0].scheduled_end, started_at: timestamp(0), lead_technician_id: ADMIN_ID, technician_ids: [ADMIN_ID], diagnostics: 'Ověřen alarm BMS-COM-031. Datová linka mezi BMS a střídačem vykazuje výpadky.', root_cause: 'Probíhá ověření kabeláže a zakončení sběrnice.', work_performed: 'Zkontrolováno napájení BMS a diagnostický log.', materials: [], measurements: [{ label: 'SOC', value: '64', unit: '%' }, { label: 'Teplota modulů', value: '24', unit: '°C' }], safety_checks: [{ label: 'Bezpečné odpojení baterie', checked: true }, { label: 'Kontrola BMS a alarmů', checked: true }], recommendations: '', next_action: 'Proměřit datovou linku a případně vyměnit komunikační kabel.', client_present: true, created_by_member_id: ADMIN_ID, created_at: timestamp(-1), updated_at: timestamp(0) },
    { id: uuid(92102), service_case_id: service_cases[2].id, visit_number: 1, status: 'completed', scheduled_start: service_cases[2].scheduled_start, completed_at: timestamp(-5), lead_technician_id: uuid(3), technician_ids: [uuid(3)], diagnostics: 'Bez aktivních alarmů.', root_cause: null, work_performed: 'Provedena kontrola modulů, dotažení spojů, kontrola BMS a test chlazení.', materials: [], measurements: [{ label: 'SOH', value: '97', unit: '%' }], safety_checks: [{ label: 'Kontrola BMS a alarmů', checked: true }, { label: 'Kontrola ventilace / chlazení', checked: true }], recommendations: 'Další kontrola za 12 měsíců.', next_action: null, client_present: true, created_by_member_id: uuid(3), created_at: timestamp(-5), updated_at: timestamp(-5) },
  ];
  const service_documents = [{ id: uuid(92201), service_case_id: service_cases[2].id, service_visit_id: service_visits[1].id, document_type: 'service_protocol', number: 'SRV-2026-0004', title: 'Servisní protokol · Preventivní kontrola bateriového úložiště', status: 'signed', document_snapshot: {}, recipient_name: 'Správa budovy', recipient_email: subjects[2].email, signed_at: timestamp(-5), signer_name: 'Správa budovy', created_at: timestamp(-5), updated_at: timestamp(-5) }];
  const service_events = service_cases.flatMap((item, index) => [{ id: uuid(92301 + index), service_case_id: item.id, event_type: 'case_created', summary: `${item.number} byl založen`, actor_member_id: ADMIN_ID, created_at: item.created_at }]);
  const tables = {
    ...
    service_cases, service_visits, service_attachments: [], service_documents, service_events,
