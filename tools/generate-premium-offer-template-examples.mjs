import fs from 'node:fs';
import path from 'node:path';
import { createRequire } from 'node:module';
import {
  AlignmentType,
  BorderStyle,
  Document,
  HeadingLevel,
  ImageRun,
  Packer,
  Paragraph,
  Table,
  TableCell,
  TableRow,
  TextRun,
  WidthType,
} from 'docx';

const require = createRequire(import.meta.url);

const root = process.cwd();
const templatePath = path.join(root, 'docs/templates/ekv_premium_offer_template.html');
const outputDir = path.join(root, 'docs/generated-examples/offer-template');
const migrationPath = path.join(root, 'supabase/migrations/20260705113000_seed_ekv_premium_offer_template.sql');

fs.mkdirSync(outputDir, { recursive: true });

const template = fs.readFileSync(templatePath, 'utf8');
const companyLogoDataUri = 'data:image/png;base64,';

const formatCurrency = (value) => `${Number(value || 0).toLocaleString('cs-CZ', {
  minimumFractionDigits: 2,
  maximumFractionDigits: 2,
})} Kč`;

const formatDate = (value) => {
  if (!value) return '-';
  return new Intl.DateTimeFormat('cs-CZ').format(new Date(value));
};

const dataUriToUint8Array = (dataUri) => {
  const base64 = String(dataUri || '').split(',')[1] || '';
  return Uint8Array.from(Buffer.from(base64, 'base64'));
};

const escapeHtml = (value) => String(value ?? '')
  .replace(/&/g, '&amp;')
  .replace(/</g, '&lt;')
  .replace(/>/g, '&gt;')
  .replace(/"/g, '&quot;')
  .replace(/'/g, '&#039;');

const sampleItems = [
  {
    position: 1,
    code: 'SET-FVE-001',
    name: 'FVE sestava 10,2 kWp - panely, konstrukce, DC část',
    description: 'Fotovoltaické panely, montážní systém, kabeláž a jištění DC části.',
    quantity: 1,
    unit: 'kpl',
    unitPrice: 248000,
    vatRate: 21,
    discountPercent: 0,
    marginTotal: 42000,
    lineTotal: 248000,
    totalWithTax: 300080,
  },
  {
    position: 2,
    code: 'INV-SOLAX-012',
    name: 'Střídač SolaX X3-Hybrid včetně konfigurace',
    description: 'Hybridní střídač, monitoring, zprovoznění a nastavení parametrů.',
    quantity: 1,
    unit: 'ks',
    unitPrice: 78000,
    vatRate: 21,
    discountPercent: 3,
    marginTotal: 12200,
    lineTotal: 75660,
    totalWithTax: 91548.6,
  },
  {
    position: 3,
    code: 'BAT-SOLAX-011',
    name: 'Bateriové úložiště 11,6 kWh',
    description: 'Modulové bateriové úložiště včetně BMS a propojovací kabeláže.',
    quantity: 1,
    unit: 'ks',
    unitPrice: 126500,
    vatRate: 21,
    discountPercent: 0,
    marginTotal: 23800,
    lineTotal: 126500,
    totalWithTax: 153065,
  },
  {
    position: 4,
    code: 'INST-FVE',
    name: 'Montáž, revize a předávací dokumentace',
    description: 'Instalace, výchozí revize, dokumentace skutečného provedení a předání.',
    quantity: 1,
    unit: 'kpl',
    unitPrice: 89000,
    vatRate: 21,
    discountPercent: 0,
    marginTotal: 31000,
    lineTotal: 89000,
    totalWithTax: 107690,
  },
];

const subtotal = sampleItems.reduce((sum, item) => sum + (item.unitPrice * item.quantity), 0);
const total = sampleItems.reduce((sum, item) => sum + item.lineTotal, 0);
const taxTotal = sampleItems.reduce((sum, item) => sum + (item.totalWithTax - item.lineTotal), 0);
const totalWithTax = total + taxTotal;
const discountTotal = subtotal - total;

const itemsTable = `<table>
  <thead>
    <tr>
      <th>#</th>
      <th>Kód</th>
      <th>Název</th>
      <th class="num">Množství</th>
      <th class="num">Jedn. cena</th>
      <th class="num">DPH</th>
      <th class="num">Sleva</th>
      <th class="num">Celkem bez DPH</th>
      <th class="num">Celkem s DPH</th>
    </tr>
  </thead>
  <tbody>
    ${sampleItems.map((item) => `<tr>
      <td>${item.position}</td>
      <td>${escapeHtml(item.code)}</td>
      <td class="item-name"><strong>${escapeHtml(item.name)}</strong><div class="muted">${escapeHtml(item.description)}</div></td>
      <td class="num">${Number(item.quantity).toLocaleString('cs-CZ')} ${escapeHtml(item.unit)}</td>
      <td class="num">${formatCurrency(item.unitPrice)}</td>
      <td class="num">${Number(item.vatRate).toLocaleString('cs-CZ')} %</td>
      <td class="num">${Number(item.discountPercent).toLocaleString('cs-CZ')} %</td>
      <td class="num">${formatCurrency(item.lineTotal)}</td>
      <td class="num">${formatCurrency(item.totalWithTax)}</td>
    </tr>`).join('\n')}
  </tbody>
</table>`;

const values = {
  document_number: 'NAB-26-VZOR',
  document_title: 'Nabídka - fotovoltaická elektrárna s bateriovým úložištěm',
  document_original_id: 'EKV-OFFER-NAB26VZOR-X9K42A',
  company_logo: companyLogoDataUri,
  document_date: formatDate('2026-07-05'),
  document_valid_until: formatDate('2026-08-04'),
  client_name: 'Vzorový klient s.r.o.',
  project_name: 'FVE - vzorová instalace',
  opportunity_title: 'FVE - rodinný dům 10,2 kWp',
  opportunity_description: 'Návrh dodávky fotovoltaické elektrárny včetně baterie, montáže, revize a předávací dokumentace.',
  total_amount: formatCurrency(total),
  subtotal: formatCurrency(subtotal),
  discount_total: formatCurrency(discountTotal),
  tax_total: formatCurrency(taxTotal),
  total_with_tax: formatCurrency(totalWithTax),
  item_count: sampleItems.length,
  items_table: itemsTable,
  notes: 'Termín realizace bude potvrzen po technické prohlídce a odsouhlasení finální konfigurace.',
  generated_at: formatDate(new Date()),
};

const fill = (html) => Object.entries(values).reduce(
  (content, [key, value]) => content.replaceAll(`{{${key}}}`, String(value)),
  html,
);

const html = fill(template);
const htmlPath = path.join(outputDir, 'ekv_premium_offer_example.html');
const pdfPath = path.join(outputDir, 'ekv_premium_offer_example.pdf');
const docxPath = path.join(outputDir, 'ekv_premium_offer_example.docx');
fs.writeFileSync(htmlPath, html, 'utf8');

const makeText = (text, options = {}) => new TextRun({
  text: String(text ?? ''),
  bold: options.bold,
  color: options.color || '111827',
  size: options.size || 20,
});

const makeParagraph = (text, options = {}) => new Paragraph({
  heading: options.heading,
  alignment: options.alignment,
  spacing: options.spacing || { after: 110 },
  children: [makeText(text, options)],
});

const makeCell = (text, options = {}) => new TableCell({
  width: options.width ? { size: options.width, type: WidthType.PERCENTAGE } : undefined,
  shading: options.shading ? { fill: options.shading } : undefined,
  margins: { top: 110, bottom: 110, left: 110, right: 110 },
  children: [new Paragraph({
    alignment: options.align || AlignmentType.LEFT,
    children: [makeText(text, {
      bold: options.bold,
      color: options.color || '111827',
      size: options.size || 18,
    })],
  })],
});

const borders = {
  top: { style: BorderStyle.SINGLE, size: 1, color: 'D8E0EB' },
  bottom: { style: BorderStyle.SINGLE, size: 1, color: 'D8E0EB' },
  left: { style: BorderStyle.SINGLE, size: 1, color: 'D8E0EB' },
  right: { style: BorderStyle.SINGLE, size: 1, color: 'D8E0EB' },
  insideHorizontal: { style: BorderStyle.SINGLE, size: 1, color: 'EDF1F7' },
  insideVertical: { style: BorderStyle.SINGLE, size: 1, color: 'EDF1F7' },
};

const itemTable = new Table({
  width: { size: 100, type: WidthType.PERCENTAGE },
  borders,
  rows: [
    new TableRow({
      tableHeader: true,
      children: [
        makeCell('#', { bold: true, shading: 'EEF3F9', width: 5 }),
        makeCell('Kód', { bold: true, shading: 'EEF3F9', width: 12 }),
        makeCell('Název', { bold: true, shading: 'EEF3F9', width: 35 }),
        makeCell('Množství', { bold: true, shading: 'EEF3F9', width: 12, align: AlignmentType.RIGHT }),
        makeCell('Jedn. cena', { bold: true, shading: 'EEF3F9', width: 16, align: AlignmentType.RIGHT }),
        makeCell('Celkem', { bold: true, shading: 'EEF3F9', width: 20, align: AlignmentType.RIGHT }),
      ],
    }),
    ...sampleItems.map((item) => new TableRow({
      children: [
        makeCell(item.position, { width: 5 }),
        makeCell(item.code, { width: 12 }),
        makeCell(item.name, { width: 35 }),
        makeCell(`${item.quantity} ${item.unit}`, { width: 12, align: AlignmentType.RIGHT }),
        makeCell(formatCurrency(item.unitPrice), { width: 16, align: AlignmentType.RIGHT }),
        makeCell(formatCurrency(item.lineTotal), { width: 20, align: AlignmentType.RIGHT }),
      ],
    })),
  ],
});

const totalsTable = new Table({
  width: { size: 58, type: WidthType.PERCENTAGE },
  alignment: AlignmentType.RIGHT,
  borders,
  rows: [
    new TableRow({ children: [makeCell('Před slevou', { width: 50 }), makeCell(formatCurrency(subtotal), { width: 50, align: AlignmentType.RIGHT })] }),
    new TableRow({ children: [makeCell('Sleva', { width: 50 }), makeCell(formatCurrency(discountTotal), { width: 50, align: AlignmentType.RIGHT })] }),
    new TableRow({ children: [makeCell('DPH', { width: 50 }), makeCell(formatCurrency(taxTotal), { width: 50, align: AlignmentType.RIGHT })] }),
    new TableRow({ children: [makeCell('Celkem s DPH', { width: 50, bold: true, shading: '153B82', color: 'FFFFFF' }), makeCell(formatCurrency(totalWithTax), { width: 50, bold: true, shading: '153B82', color: 'FFFFFF', align: AlignmentType.RIGHT })] }),
  ],
});

const signatureTable = new Table({
  width: { size: 100, type: WidthType.PERCENTAGE },
  borders,
  rows: [
    new TableRow({
      children: [
        makeCell('Vzorový klient s.r.o.\n\n\nDatum, jméno a podpis', { width: 50 }),
        makeCell('EKV Project s.r.o.\n\n\nDatum, jméno a podpis', { width: 50 }),
      ],
    }),
  ],
});

const doc = new Document({
  sections: [{
    properties: { page: { margin: { top: 720, right: 720, bottom: 720, left: 720 } } },
    children: [
      new Paragraph({ spacing: { after: 100 }, children: [new ImageRun({ data: dataUriToUint8Array(companyLogoDataUri), transformation: { width: 185, height: 33 } })] }),
      makeParagraph('Nabídka NAB-26-VZOR', { bold: true, size: 36, spacing: { after: 80 } }),
      makeParagraph('FVE - rodinný dům 10,2 kWp', { size: 24, color: '334155', spacing: { after: 120 } }),
      makeParagraph('Klient: Vzorový klient s.r.o.', { bold: true }),
      makeParagraph('Projekt: FVE - vzorová instalace'),
      makeParagraph(values.opportunity_description),
      makeParagraph('Položkový rozpočet', { heading: HeadingLevel.HEADING_2, bold: true, size: 25, spacing: { before: 220, after: 100 } }),
      itemTable,
      makeParagraph('Souhrn nabídky', { heading: HeadingLevel.HEADING_2, bold: true, size: 25, spacing: { before: 220, after: 100 } }),
      totalsTable,
      makeParagraph('Akceptace nabídky', { heading: HeadingLevel.HEADING_2, bold: true, size: 25, spacing: { before: 260, after: 100 } }),
      signatureTable,
      makeParagraph(`Vygenerováno: ${values.generated_at} | ID origin?lu: ${values.document_original_id}`, { color: '667085', size: 17, spacing: { before: 260 } }),
    ],
  }],
});

fs.writeFileSync(docxPath, await Packer.toBuffer(doc));

try {
  const { chromium } = require('C:/tmp/pw-live-test/node_modules/playwright');
  const browser = await chromium.launch({ headless: true });
  const page = await browser.newPage({ viewport: { width: 1440, height: 1800 } });
  await page.goto(`file:///${htmlPath.replaceAll('\\', '/')}`, { waitUntil: 'load' });
  await page.pdf({
    path: pdfPath,
    format: 'A4',
    printBackground: true,
    margin: { top: '0mm', right: '0mm', bottom: '0mm', left: '0mm' },
  });
  await browser.close();
} catch (error) {
  console.warn(`PDF generation skipped: ${error.message}`);
}

const escapedTemplate = template.replaceAll("'", "''");
const migration = `-- Seed premium EKV offer template for CRM offer PDF/DOCX generation.
-- Idempotent by template name; safe even when order_templates.name has no unique constraint.
do $$
begin
  if exists (select 1 from public.order_templates where name = 'EKV Premium nabidka FVE') then
    update public.order_templates
       set description = 'Moderni nabidka pro PDF vystup se souhrnem, polozkami, ID originalu a podpisovym blokem. DOCX vystup pouziva specializovany renderer v aplikaci.',
           document_category = 'offer',
           content = '${escapedTemplate}',
           is_active = true,
           updated_at = coalesce(now(), updated_at)
     where name = 'EKV Premium nabidka FVE';
  else
    insert into public.order_templates (name, description, document_category, content, is_active)
    values (
      'EKV Premium nabidka FVE',
      'Moderni nabidka pro PDF vystup se souhrnem, polozkami, ID originalu a podpisovym blokem. DOCX vystup pouziva specializovany renderer v aplikaci.',
      'offer',
      '${escapedTemplate}',
      true
    );
  end if;
end $$;
`;

fs.writeFileSync(migrationPath, migration, 'utf8');

console.log(JSON.stringify({
  html: htmlPath,
  pdf: fs.existsSync(pdfPath) ? pdfPath : null,
  docx: docxPath,
  migration: migrationPath,
}, null, 2));
