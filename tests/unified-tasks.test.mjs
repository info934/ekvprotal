...
test('linked tasks appear once; standalone project tasks remain accessible',()=>{assert.equal(rows.length,4);assert(!rows.some(t=>t.name==='Duplicate'));assert(rows.find(t=>t.name==='Montáž').path.startsWith('/realizace/r1#plan'));assert(rows.find(t=>t.name==='Samostatný').path.includes('?task=unlinked'));});
