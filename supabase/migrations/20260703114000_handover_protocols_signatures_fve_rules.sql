...
values
  ('Předávací protokol - celkový', 'Výchozí šablona pro celkové předání realizace.', 'handover_full', '<h1>Předávací protokol {document_number}</h1><p>Klient: {client_name}</p><p>Projekt: {project_name}</p><p>Realizace: {realization_name}</p><h2>Rozsah předání</h2><p>{handover_scope}</p><h2>Předané položky</h2>{items_table}<h2>Vady a nedodělky</h2>{defects_table}<h2>Podpisy</h2>{signatures_table}', true),
  ('Předávací protokol - částečný', 'Výchozí šablona pro částečné předání.', 'handover_partial', '<h1>Částečný předávací protokol {document_number}</h1><p>Klient: {client_name}</p><p>Projekt: {project_name}</p><h2>Předávaná část</h2><p>{handover_scope}</p>{items_table}{defects_table}{signatures_table}', true),
  ('Servisní protokol', 'Výchozí šablona servisního zásahu.', 'service_protocol', '<h1>Servisní protokol {document_number}</h1><p>Klient: {client_name}</p><p>Projekt: {project_name}</p><h2>Popis servisního zásahu</h2><p>{service_description}</p><h2>Položky</h2>{items_table}<h2>Podpisy</h2>{signatures_table}', true),
  ('Smlouva', 'Výchozí jednoduchá smluvní šablona.', 'contract', '<h1>Smlouva {document_number}</h1><p>Klient: {client_name}</p><p>Projekt: {project_name}</p><p>{handover_scope}</p><h2>Položky / předmět smlouvy</h2>{items_table}<h2>Podpisy</h2>{signatures_table}', true)
on conflict do nothing;
