-- Preserve blocked state between project tasks and the planning board.
-- project_tasks.status is text; no destructive constraint replacement is required.
begin;

insert into public.task_statuses (name) values ('Blokováno') on conflict (name) do nothing;
...
begin
  ...
  v_status := case
    ...
    when new.status = 'blocked' then 'Blokováno'
    when new.status = 'in_progress' then 'V řešení'
    else 'Nové'
  end;
...
-- Keep all planning details during backfill; do not run the reverse sync.
select set_config('app.planning_sync', '1', true);
update public.project_tasks task set status = 'Blokováno'
from public.planning_items item
where item.legacy_project_task_id = task.id and item.status = 'blocked'
and task.status is distinct from 'Blokováno';
