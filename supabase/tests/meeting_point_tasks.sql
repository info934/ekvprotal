...
END $$;
DO $$
DECLARE note_id uuid:=current_setting('test.kd_id')::uuid; plan uuid:=current_setting('test.kd_plan')::uuid; member uuid; r public.meeting_notes; before_count integer;
BEGIN
 SELECT id INTO member FROM public.members WHERE auth_user_id=current_setting('test.kd_admin')::uuid;
 r:=public.save_meeting_note(note_id,plan,2,'KD task',CURRENT_DATE,'','[{"kind":"decision","text":"Prověřit trasu"}]');
 SELECT count(*) INTO before_count FROM public.planning_items WHERE plan_id=plan;
 r:=public.create_meeting_point_task(note_id,3,0,'Prověřit trasu',CURRENT_DATE+1,member);
 IF r.version<>4 OR r.points->0->>'kind'<>'task' OR NOT EXISTS(SELECT 1 FROM public.planning_items WHERE id=(r.points->0->>'planning_item_id')::uuid AND plan_id=plan AND member_id=member) THEN RAISE EXCEPTION 'Task not linked atomically';END IF;
 BEGIN
  PERFORM public.create_meeting_point_task(note_id,3,0,'Duplicate',CURRENT_DATE+1,member);
  RAISE EXCEPTION 'Duplicate accepted' USING ERRCODE='23514';
 EXCEPTION WHEN serialization_failure THEN NULL; END;
 IF (SELECT count(*) FROM public.planning_items WHERE plan_id=plan)<>before_count+1 THEN RAISE EXCEPTION 'Duplicate task created';END IF;
 BEGIN
  PERFORM public.create_meeting_point_task(note_id,4,0,'Duplicate',CURRENT_DATE+1,member);
  RAISE EXCEPTION 'Existing link overwritten' USING ERRCODE='23514';
 EXCEPTION WHEN raise_exception THEN NULL;END;
END $$;
